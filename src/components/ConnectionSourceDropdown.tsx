...
export function ConnectionSourceDropdown<T extends string>({ value, options, onChange }: ConnectionSourceDropdownProps<T>) {
  ...
  return (
    <>
      ...
      >
        <Text style={[styles.title, { color: colors.text.ink }]} numberOfLines={1}>
          {selected?.label ?? '连接'}
        </Text>
        <ChevronDown size={16} color={colors.text.soft} />
        {selected ? <ConnectionStatusDot status={selected.status} /> : null}
      </Pressable>
          ...
          <View style={[styles.sheet, { backgroundColor: colors.bg.canvas }]}>
            ...
            {options.map((option) => {
              ...
              return (
                ...
                >
                  <ConnectionStatusDot status={option.status} size={10} />
                  <View style={styles.optionText}>
                    <Text style={[styles.optionLabel, { color: colors.text.ink }]}>{option.label}</Text>
