...
class RelayConnection {
  ...
  constructor(private readonly relay: RemoteRelayEndpoint) {}
  ...
  private handleFrame(frame: RelayInboundFrame) {
    ...
    if (frame.type === 'chunk') {
      ...
      try {
        const text = pending.textDecoder.feed(bytes);
  ...
  private completeJson(pending: JsonPending<unknown>) {
    try {
      ...
      const payload = parseJsonText(text, pending.status ?? undefined);
