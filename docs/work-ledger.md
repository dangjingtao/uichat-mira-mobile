...
- MOB-017 已判定**有条件通过（放行后续）**：PR #56 已完成 `RecordingAdapter` 可替换边界、Android `MediaRecorder` / iOS `AVAudioRecorder` 原生录音、两端麦克风权限、App 私有目录落盘、重启后本地草稿恢复 UI 与单测；iOS Objective-C++ 编译问题已修复。Mobile CI #623 全绿（typecheck / lint / Jest、Android debug、iOS simulator、iOS unsigned device）。OpenCode Review 的 `spawnSync opencode E2BIG` 属于评审基建参数长度问题，不是代码 finding。真机短录音重启恢复与约 40 分钟长录音继续挂最终人工验收，不阻塞后续拾言施工。
...
| MOB-017 | 拾言录音与本地恢复 | Mobile；RecordingAdapter、麦克风权限、本地草稿恢复、40 分钟录音 smoke | **有条件通过（放行后续）**：PR #56 代码与自动化 Gate 已通过；Mobile CI #623 全绿；真机短录音重启恢复与约 40 分钟长录音挂人工验收 | `mob_017_shiyan_recording_local_recovery` | 无 Cloud 依赖 |
...
- **MOB-017**：Mobile 录音 Adapter / 本地恢复 / 原生麦克风能力，已 Gate / 有条件通过。
...
当前优先并行 MOB-014 与 MOB-019。任何 Builder 在开始前仍必须按派卡 Skill 读取当前仓库事实和实际 HEAD，旧派卡不得覆盖新代码事实。
