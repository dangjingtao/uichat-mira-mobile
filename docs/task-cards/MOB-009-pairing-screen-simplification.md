...
### 真机人工验收：2026-09-04 通过

五条路径在真实 Mira Desktop + 真实 Android/iOS 设备上覆盖完成：
...
注：路径 1、3、5 由同批次合入的 MOB-036 真机 smoke 顺验覆盖（扫码 → Sheet → 关闭、扫码 → 提交 → Desktop 批准 → Toast → 首页、Desktop 拒绝 / 请求过期）；路径 2（粘贴 → Sheet → 提交）由本次独立真机跑验证，确认兜底输入与扫码走同一 `parsePairingUriV1()` / 授权流。MOB-009 从「有条件完成」升级为「完全完成」。
