...
拾言的产品定义、范围、核心流程、任务状态语义、关键数据边界与跨仓库合同，统一以本目录为唯一真相源。
...
如果实现需要改变本页定义，必须先更新 Mobile 侧真相，再修改其它仓库。
...
3. 若会改变产品或跨仓库合同，先修改本文件并完成评审。

### 修改后

1. 对照本文件逐项核对行为和状态语义。
2. 检查 `mira-shiyan-cloud` 是否出现与本文件冲突的接口、状态或数据定义。
3. 检查 `mira-shiyan` 是否被错误用作业务数据库或业务真相源。
4. 若跨仓库合同发生变化，三个仓库中的引用必须同步更新。

一致性检查发现冲突时，以本文件为准；不得在下游仓库通过“实现已经这样了”反向覆盖产品真相。

## 12. 相关文档

早期技术草案：`../meeting-capture-mvp.md`。

该文档保留讨论与技术探索价值；与本页冲突时，以本页为准。
